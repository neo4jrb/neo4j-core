    ...
    describe CypherResponse do
      ...
      describe '#to_struct_enumeration' do
        it 'creates a enumerable of hash key values' do
          ...
          expect(response.to_struct_enumeration.to_a).to eq([hash_to_struct(response, 'ID(n)'.to_sym => 0)])
        end
      ...
      describe '#to_node_enumeration' do
        it 'returns basic values' do
          ...
          expect(response.to_node_enumeration.to_a).to eq([hash_to_struct(response, 'person.name'.to_sym => 'Billy'), hash_to_struct(response, 'person.name'.to_sym => 'Jimmy')])
        end
