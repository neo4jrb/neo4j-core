...
describe Neo4j::Core::EventHandler, :type => :integration  do
  ...
  before { new_tx}
  after  { Neo4j.event_handler.remove(@rec) if @rec; finish_tx }
  ...
  it "#classes_changed should be called once for all added nodes" do
    model1 = Neo4j::Node.new(:name => 'one', :_classname => 'TestModel')
    model2 = Neo4j::Node.new(:name => 'one', :_classname => 'TestModel')
