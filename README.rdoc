...
{Neo4j::Cypher} Cypher Query Generator

== Example of Cypher DSL

Convensions:
* Symbols represents not bound variables.
* Strings represents constants, like relationship types.
* Variables names (n0, n1, ...) are by default generated (which can be overridden)
...
See RSpecs:
  ...
  Neo4j::Cypher.new do
    ...
    match n <=> :x
    ret :x
  end.to_s.should == "START n0=node(3) MATCH (n0)--(x) RETURN x"

By using Ruby local variables (n in the example above) we can get help from the editor (e.g. RubyMine) and syntax checks for free.
...
==== START n0=node(3) MATCH (n0)-[:blocks]->(x) RETURN x"
